...
// Compute for shadow if hasShadow is true.
    ...
    for (int i = 0; i < scene.numPtLights; i++) {
        Vector3d L = (scene.ptLight[i].position - nearestHitRec.p).makeUnitVector();
        float ks = 1.0; // k-shadow modifier
        if (hasShadow) {
            Ray shadowRay(nearestHitRec.p, L);
            double tMax = (scene.ptLight[i].position - nearestHitRec.p).length();
            for (int n = 0; n < scene.numSurfaces; n++) {
                if (scene.surfacep[n]->shadowHit(shadowRay, DEFAULT_TMIN, tMax)) {
                    ks = 0;
                    break;
                }
            }
        }
        result += ks * computePhongLighting(L, N, V, *nearestHitRec.mat_ptr, scene.ptLight[i]);
    }
...
// Add to result the reflection of the scene.
    ...
    for (int i = 0; i < reflectLevels; i++) {
        ...
        result += nearestHitRec.mat_ptr->k_rg * Raytrace().TraceRay(
