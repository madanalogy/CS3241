...
static const int imageWidth2 = 1280;
static const int imageHeight2 = 800;
...
// Define materials.
    ...
    // Light blue.
    scene.material[1].k_d = Color( 0.4f, 0.4f, 0.8f ) * 0.9f;
    scene.material[1].k_a = scene.material[1].k_d;
    scene.material[1].k_r = Color( 0.8f, 0.8f, 0.8f ) / 1.5f;
    scene.material[1].k_rg = Color( 0.8f, 0.8f, 0.8f ) / 2.5f;
    scene.material[1].n = 64.0f;

    // Dark Green.
    scene.material[2].k_d = Color( 0.15f, 0.4f, 0.15f );
    scene.material[2].k_a = scene.material[2].k_d;
    scene.material[2].k_r = Color( 0.8f, 0.8f, 0.8f );
    scene.material[2].k_rg = Color( 0.8f, 0.8f, 0.8f ) / 3.0f;
    scene.material[2].n = 64.0f;

    // Gray.
    scene.material[3].k_d = Color( 0.6f, 0.6f, 0.6f );
    scene.material[3].k_a = scene.material[3].k_d;
    scene.material[3].k_r = Color( 0.6f, 0.6f, 0.6f ) / 1.5f ;
    scene.material[3].k_rg = Color( 0.8f, 0.8f, 0.8f ) / 3.0f;
    scene.material[3].n = 128.0f;
...
// Define point light sources.
    ...
    scene.ptLight[0].I_source = Color( 1.0f, 1.0f, 1.0f ) * 0.5f;
    scene.ptLight[0].position = Vector3d( 150.0, 120.0, 35.0 );

    scene.ptLight[1].I_source = Color( 1.0f, 1.0f, 1.0f ) * 0.5f;
    scene.ptLight[1].position = Vector3d( 35.0, 120.0, 150.0 );


// TODO: Define surface primitives.
    ...
    scene.surfacep[2] = new Plane( 0.0, 0.0, 1.0, 0.0, &(scene.material[1]) ); // Right vertical plane.
    scene.surfacep[3] = new Sphere( Vector3d( 45.0, 15.0, 85.0 ), 15.0, &(scene.material[0]) ); // left sphere.
    scene.surfacep[4] = new Sphere( Vector3d( 85.0, 15.0, 45.0 ), 15.0, &(scene.material[1]) ); // right sphere.

    // Pyramid -z (top-right) face.
    scene.surfacep[5] = new Triangle( Vector3d( 15.0, 0.0, 15.0 ), Vector3d( 32.25, 25.75, 32.25 ),
                                      Vector3d( 50.0, 0.0, 15.0 ), &(scene.material[2]) );

    // Pyramid +x (bot-right) face.
    scene.surfacep[6] = new Triangle( Vector3d( 50.0, 0.0, 15.0 ), Vector3d( 32.25, 25.75, 32.25 ),
                                      Vector3d( 50.0, 0.0, 50.0 ), &(scene.material[2]) );

    // Pyramid +z (bot-left) face.
    scene.surfacep[7] = new Triangle( Vector3d( 50.0, 0.0, 50.0 ), Vector3d( 32.25, 25.75, 32.25 ),
                                      Vector3d( 15.0, 0.0, 50.0 ), &(scene.material[2]) );

    // Pyramid +x (top-left) face.
    scene.surfacep[8] = new Triangle( Vector3d( 15.0, 0.0, 50.0 ), Vector3d( 32.25, 25.75, 32.25 ),
                                       Vector3d( 15.0, 0.0, 15.0 ), &(scene.material[2]) );


// Define camera.

    scene.camera = Camera( Vector3d( 150.0, 120.0, 150.0 ), Vector3d( 55.0, 27.5, 55.0 ), Vector3d( 0.0, 1.0, 0.0 ),
                           (-1.0 * imageWidth) / imageHeight, (1.0 * imageWidth) / imageHeight, -1.0, 1.0, 3.0,
                           imageWidth, imageHeight );
}
