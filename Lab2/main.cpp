...
void DrawOneCar( float bodyColor[3] )
{
    ...
    glPushMatrix();
        glScalef(1.0, 1.0, CAR_HEIGHT / CAR_WIDTH);
        glTranslatef(0,0,CAR_WIDTH / 2);
        glutSolidCube(CAR_WIDTH);
    glPopMatrix();
    glPushMatrix();
        glScalef(CAR_LENGTH / CAR_WIDTH, 1.0, 0.5);
        glTranslatef(0,0,CAR_WIDTH / 2);
        glutSolidCube(CAR_WIDTH);
    glPopMatrix();
    ...
    glPushMatrix();
        glTranslatef((CAR_LENGTH/2) - (CAR_LENGTH/6),CAR_WIDTH/2,CAR_HEIGHT/4);
        glRotatef(90, 1.0, 0, 0);
        glutSolidTorus(CAR_HEIGHT/8,CAR_HEIGHT/4,12,20);
    glPopMatrix();
    glPushMatrix();
...
void DrawAllCars( void ) 
{
    ...
    {
        double x = car[i].xzAxis[0];    //x-coord of great circle V, constant
        double z = car[i].xzAxis[1];    //z-coord of great circle V, constant
        double rot = car[i].rotAngle;    //Used to define great circle, constant
        double ang = car[i].angularPos;  //Coords should depend on this, 0-360
        //len = length of vector xz
        double len = sqrt(pow(x,2) + pow(z,2));
        //theta = angle of xz from z axis
        double theta = acos(z/len);
        //tPrime = 90 Degree rotation of xz
        double tPrime = atan(abs(z/x)) + PI/4;
        //Oth coords = x & z orthogonal to original xz
        double xOth = len * sin(tPrime);
        double zOth = len * cos(tPrime);
        //Rot coords = Oth vector rotated around original xz vector
        double alpha = rot*PI/180;
        double xRot = (1-cos(alpha))*((x*xOth+z*zOth)/(pow(x,2)+pow(z,2)))*x
                + xOth*cos(alpha);
        double yRot = sin(alpha)*(z*xOth-zOth*x)/len;
        double zRot = (1-cos(alpha))*((x*xOth+z*zOth)/(pow(x,2)+pow(z,2)))*z
                + zOth*cos(alpha);
        //Cross coords = cross product of rotated vector with original vector
        double xCross = -(z * yRot);
        double yCross = (z * xRot) - (x * zRot);
        double zCross = (x * yRot);

        glPushMatrix();
            glRotated(ang, xCross, yCross, zCross);
            glRotated(rot,x,0,z);
...
void MyDisplay( void )
{
    ...
    //***********************************************************************
    gluLookAt( eyeDistance * cos(eyeLatitude*PI/180) * cos(eyeLongitude*PI/180),
